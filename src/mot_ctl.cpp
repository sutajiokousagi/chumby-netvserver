...
void ioctl_reset(int file_desc)
{
  ...
  if (ret_val < 0) {
    printf("ioctl_reset failed:%d", ret_val);
    exit(-1);
  }
}
...
void ioctl_led0(int file_desc, int state) {
  ...
  if (ret_val < 0) {
    printf("ioctl_led0 failed:%d", ret_val);
    exit(-1);
  }
}
...
void ioctl_led1(int file_desc, int state) {
  ...
  if (ret_val < 0) {
    printf("ioctl_led1 failed:%d", ret_val);
    exit(-1);
  }
}
...
void ioctl_done(int file_desc, int *state) {
  ...
  if (ret_val < 0) {
    printf("ioctl_done failed:%d", ret_val);
    exit(-1);
  }
}
...
void ioctl_init(int file_desc, int *state) {
  ...
  if (ret_val < 0) {
    printf("ioctl_init failed:%d", ret_val);
    exit(-1);
  }
}
...
int dump_registers(int stats) {
    ...
    if( stats ) {
      ...
      printf( "Select stats (as of version %d): ", STATS_CORRECT_MAJOR );
...
int dump_register(unsigned char address) {
  ...
  printf("0x%02lx: %02hx", (unsigned long) address, buffer);
